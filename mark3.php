...
<br><br>
                ...
                <div class="row">
							...
							$result = $con->query("SELECT * FROM  markall where RollNo='$a' and exam='$b'");
							 
							  if ($result->num_rows > 0) {

							mysqli_query($con,"UPDATE markall SET `$d` = '$c'
								 WHERE `RollNo` = '$a' and `exam`='$b'");
								 echo "<script> alert ('Mark UPDATE Successfull for $a in $d($c) in $b ');</script>";
							  }
							  else{
							mysqli_query($con,"INSERT INTO markall (RollNo,exam,`$d`) VALUES ('$a', '$b', '$c')");
