...
<?php
	...
	mysql_select_db("schoolmanagement") or die(mysql_error());
	   ...
	   $res  = mysql_query("SELECT * FROM `teacherreg` WHERE `Initial`='".$id."' ") 
		or die(mysql_error());
	 ...
	 {
		 $_SESSION['teacher'] = $id ;
		 $_SESSION['teachername']=$Pass['Name'];
		 ...
		 echo "Login Successful";
		  ...
		  if($Password != $Pass['password'] ){
			echo "<script> alert ('Username And password did not match ');</script>";
			 ...
			 echo "<script> alert ('Needs Admins Approval ');</script>";
		 }
			
			echo "<h2 align='center'><a  href=teacherlog.php> back To Teacher </a></h2>";
